...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    locals.forEach(async item => {
        ...
        if (item.typeMonitoring !== 'perimetral') {
            ...
            if (getLocal.typeMonitoring !== 'perimetral') {
                local.push(getLocal);
    ...
    document.querySelector('.mainContain').addEventListener('submit', e => {
        ...
        if (keySubmit) {
            ...
            axios.get(`https://${'72.68.60.254'}:4000/bot/voice/text=${`corte realizado por ${JSON.parse(localStorage.getItem('appManagerUser')).username}`}/type=simple`);
                    ...
                    html2canvas(item)
                        .then(canvas => {
                            ...
                            axios.post(URL_IMGTEXT, formData)
                                .then(result => {
                                    console.error(`POST ${`${'72.68.60.254'}:4000/bot/img`} ${result.status}`);
                                    item.remove();
...
function printDivData(arrayData, elementHtml, callback) {
    ...
    arrayData.forEach(data => {
        ...
        printDataDish('promedio de procesos', data.resultEvaluation.processAverage, firts, data.name);
...
function printDataDish(keyData, data, elementHtml, name) {
...
function LiveYarvis(callback) {
    axios.get(`https://${'72.68.60.254'}:4000/`)
        .then(result => {
            callback();
        })
        .catch(err => {
