...
routerUser.get(`${nameApi}/user`, validateSessionAndUserSuper, async (req, res) => {
    try{
        ...
        if(req?.query?.id){
            ...
            if(!user) res.status(404).json({ error: 'Not found', status:404, mmesage: 'The user does not exist.' })
        ...
        const users = await UserModel.find({}).sort({ createdOn: -1 }).skip(skip).limit(limit)
